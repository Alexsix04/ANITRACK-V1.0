    ...
    <section class="w-full p-4 md:p-6 mt-8 flex flex-col md:flex-row gap-6 md:gap-8">
        ...
        <div class="flex-1">
            ...
            <div class="container mx-auto p-6">
                ...
                @if (empty($themes))
                    <p class="text-gray-500">No se encontraron temas para este anime.</p>
                @else
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($themes as $theme)
                            <div class="bg-white p-4 rounded-lg shadow">
                                <h2 class="font-semibold text-xl mb-2">
                                    {{ $theme['type'] }} @if ($theme['sequence'])
                                        {{ $theme['sequence'] }}
                                    @endif
                                </h2>

                                <p><strong>Canción:</strong> {{ $theme['song']['title'] }}</p>

                                @if (!empty($theme['song']['artist']))
                                    <p><strong>Artista:</strong> {{ $theme['song']['artist'] }}</p>
                                @endif

                                @if (!empty($theme['videos']))
                                    <div class="mt-2">
