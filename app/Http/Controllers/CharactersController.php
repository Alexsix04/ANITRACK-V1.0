    ...
    public function show($animeId, $characterId)
{
    ...
    if (preg_match_all('/(?:__|\*\*)\s*([A-Za-z0-9\s\'\-\(\)]+)\s*(?:__|\*\*)?:\s*([^\n]+)/', $rawDescription, $matches, PREG_SET_ORDER)) {
        foreach ($matches as $match) {
            ...
            // Limpiar delimitadores tipo ~! ... !~ pero conservar el contenido dentro del atributo
            ...
            // Asegurar que "Relative(s)" siempre se traduzca como "Familiares"
            if (strtolower($key) === 'relative(s)') {
                $translatedKey = 'Familiares';
            }
    ...
    // Eliminar TODO el contenido marcado como ~! ... !~, incluyendo enlaces dentro
    $cleanDescription = preg_replace('/~!.*?!~/', '', $cleanDescription);
    $cleanDescription = trim($cleanDescription);

    // Reemplazar enlaces de personajes en la descripción general (solo fuera de spoilers)
    ...
    // ROMPER enlaces Markdown de manga u otros (solo texto, no afecta personajes)
    $cleanDescription = preg_replace('/\[(.*?)\]\((?!https:\/\/anilist\.co\/character\/)[^\)]*\)/', '$1', $cleanDescription);
