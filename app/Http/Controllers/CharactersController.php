    ...
    public function show($animeId, $characterId)
{
    ...
    // Detectar atributos en formato __ o ** (markdown)
    if (preg_match_all('/(?:__|\*\*)([^:_]+):(?:__|\*\*)\s*([^\n]+)/', $rawDescription, $matches, PREG_SET_ORDER)) {
        ...
        // Eliminar las líneas con atributos del cuerpo de la descripción
        $cleanDescription = preg_replace('/(?:__|\*\*)([^:_]+):(?:__|\*\*)\s*[^\n]+/', '', $rawDescription);
    }
    ...
    //  Añadir datos fijos del personaje si existen (y no están ya en la descripción)
    ...
    if (!empty($characterData['dateOfBirth']) &&
